...
for fgsm_type in ["FGSM1", "FGSM2"]:
    ...
    # PATHS
    if fgsm_type == "FGSM1":
        orig_dir = "Original_Dataset_kp/king_penguin"
        adv_dir = "Attacked_Dataset1_alone/king_penguin"
        restored_dir_avg = "Restored_Images1_avg/king_penguin"
        restored_dir_med = "Restored_Images1_med/king_penguin"
        fingerprint_avg_path = "Averages1/avg_perturbation.pt"
        fingerprint_med_path = "Averages1/med_perturbation.pt"
    else:
        orig_dir = "Original_Dataset_kp/king_penguin"
        adv_dir = "Attacked_Dataset2_alone/king_penguin"
        restored_dir_avg = "Restored_Images2_avg/king_penguin"
        restored_dir_med = "Restored_Images2_med/king_penguin"
        fingerprint_avg_path = "Averages2/avg_perturbation.pt"
        fingerprint_med_path = "Averages2/med_perturbation.pt"
    ...
    print(f"{fgsm_type} Adversarial images accuracy:  {acc_adv:.2f}% ({c_adv}/{t_adv})")
    ...
    print(f"Restored {fgsm_type} (avg) accuracy:      {acc_avg:.2f}% ({c_avg}/{t_avg})")
    print(f"Restored {fgsm_type} (med) accuracy:      {acc_med:.2f}% ({c_med}/{t_med})")
